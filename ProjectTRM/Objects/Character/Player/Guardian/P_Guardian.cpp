...
// 描画処理
void P_Guardian::Draw() const
{
	//召喚陣描画
	if (now_state == State::Summon)
	{
		DrawRotaGraphF(location.x, location.y + 50, 0.5, 0.0, effect_image, TRUE, flip_flag);
	}
	...
	if (Anim_count <= anim_max_count)
	{
		...
		DrawRotaGraphF(location.x, location.y - 5.0, 1.3, 0.0, image, TRUE, flip_flag);
